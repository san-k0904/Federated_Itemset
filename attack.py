...
# --- Helper: sample data (same as secure code) ---
...
# --- Child process entrypoint (unchanged) ---
...
# --- Attack simulation + federated run ---
def run_attack_simulation(num_clients=5, use_output_saver=False):
    ...
    random.seed(int(time.time()) % 100000)  # optional reproducible-ish seed
    ...
    for idx, (p, parent_conn) in enumerate(processes, start=1):
        try:
            ...
            if isinstance(msg, tuple) and msg[0] == "error":
                ...
                # 🚨 Attack injection: tamper with Client 3’s envelope
